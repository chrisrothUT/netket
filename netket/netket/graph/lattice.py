...
class Lattice(NetworkX):
    ...
    def __init__(
        ...
        *,
        ...
        self._unit_cell = _np.expand_dims(self.extent,-1)*self.basis_vectors 
    ...
    def rotation_perm(self,period,axes=[0,1]):
      ...
      """
      Finds permutation corresponding to rotation with angle 2pi/period about specified axes
      Returns an error if the lattice does not map to itself under the transformation

      args:
        period: Period of the rotation s.t. angle=2pi/period
        axes: The axes that define the plane of rotation
      """

      perm = []
      rotation_matrix = [[_np.cos(2*m.pi/period),-_np.sin(2*m.pi/period)],[_np.sin(2*m.pi/period),_np.cos(2*m.pi/period)]]
      for coord in self._atoms_coord:
        new_coord = coord.copy()
        new_coord[axes] = _np.matmul(rotation_matrix,new_coord[axes])
        searching = 1
        while searching:
          for i in range(len(self._atoms_coord)):
            pbc = product(range(-1,2),repeat=len(self.basis_vectors))
            for trans in pbc:
              moved_coord = self._atoms_coord[i] + _np.sum(self._unit_cell*_np.expand_dims(_np.asarray(trans),1),0)
              if _np.all(_np.isclose(moved_coord, new_coord)):
                perm.append(i)
                searching = 0

          if searching:
            raise ValueError("No rotation found with desired period about specified axes")
        
      return _np.asarray(perm)
