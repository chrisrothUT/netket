...
class GCNN(nn.Module):
    ...
    def __call__(self, x_in):
        ...
        characters = np.conj(self.characters).reshape(1, 1, -1)
        x = (x*characters).reshape(x.shape[0],-1)

        x_max = jax.numpy.max(x, axis=-1 ,keepdims=True)
        x = jax.numpy.log(jax.numpy.sum(jax.numpy.exp(x - x_max), axis=-1)) + jax.numpy.squeeze(x_max, axis=-1)

        x = jnp.sum(x, axis=-1) / np.sqrt(x.shape[-1])

        return x 

 class GCNN_FT(nn.Module):
    """Implements a Group Convolutional Neural Network (G-CNN) that outputs a wavefunction
    that is invariant over a specified symmetry group using fourier transform layers.
    ...
    The G-CNN alternates convolution operations with pointwise non-linearities. The first
    layer is symmetrized linear transform given by DenseSymm, while the other layers are
    G-convolutions given by DenseEquivariantFT. The hidden layers of the G-CNN are related by
    the following equation:

    .. math ::

        {\bf f}^{i+1}_h = \Gamma( \sum_h W_{h g^{-1}} {\bf f}^i_h).
    ...
    """Number of features in each layer starting from the input. If a single number is given,
    all layers will have the same number of features."""
    irreps: Optional[Tuple[HashableArray]] = None
    """Irreducible representations of symmetry group generated by PermutationGroup.irrep_matrices()"""
    characters: Optional[HashableArray] = None
    """Desired characters of output"""
    dtype: Any = np.float64
    """The dtype of the weights."""
    ...
    def setup(self):
        ...
        if self.irreps is None and not isinstance(
            self.symmetries, PermutationGroup
        ):
            raise AttributeError(
                "irreps must be specified if symmetries are given as an array"
            )

        if self.irreps is None:
            irreps = HashableArray(self.symmetries.irrep_matrices())
        else:
            irreps = self.irreps
        ...
        self.equivariant_layers = [
            nknn.DenseEquivariantFT(
                symmetry_info=irreps,
                in_features=feature_dim[layer],
                out_features=feature_dim[layer + 1],
                use_bias=self.use_bias,
                dtype=self.dtype,
    ...
    @nn.compact
    def __call__(self, x_in):
        x = self.dense_symm(x_in).reshape(-1,self.features,self.n_symm)
